...
---
#### （6）、命名规则
  **目录命名**：存放视频数据的目录名，例：actor/number-title ,  **actor/不可更改**，后面的可以自定义，两个选项之间需要**短横杠-**。<br>
  **视频标题**：nfo中的标题命名。例：number-title。可以自定义，两个选项之间需要**短横杠-**。<br>
  **可选项**为title（片名）、actor（演员）、studio（公司）、director（导演）、release（发售日）、year（发行年份）、numbe（番号）、runtime（时长）<br>
